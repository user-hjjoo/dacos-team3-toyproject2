...
★ 표시된 부분은 팀원이 직접 확인/결정해야 하는 지점입니다.
"""

import pandas as pd
import numpy as np

DATA_PATH = 'temp_step1_output.csv'  # step1의 산출물인 전처리 데이터셋 로드
...
def log_transform_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - 팀원1의 실제 데이터로 바뀌면 왜도 수치가 달라질 수 있으므로, 실행할 때마다
      출력되는 결과를 보고 변환 대상이 여전히 타당한지 확인할 것
    """
    df = df.copy()
...
def create_derived_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    파생 변수 생성
    ★★★ [확인 필요 2] review_content 컬럼명이 팀원1 산출물에도 동일한지 확인 ★★★
    (about_product를 쓸지 review_content를 쓸지도 팀 합의 필요 -> 지금은 review_content 기준)
    """
    df = df.copy()
    df['review_length'] = df['review_content'].fillna('').apply(len)

    # ★★★ [확인 필요 3] 감성분석(sentiment_score) 포함 여부 ★★★
    # "여유되면 추가" 항목이었음. 포함하려면 아래 주석 해제 + VADER 등 설치 필요
    # from nltk.sentiment.vader import SentimentIntensityAnalyzer
    # sia = SentimentIntensityAnalyzer()
    # df['sentiment_score'] = df['review_content'].fillna('').apply(
...
def check_missing(df: pd.DataFrame, required_cols: list) -> pd.DataFrame:
    """
    ★★★ [확인 필요 4] 결측치를 제거하지 않고 경고만 출력하는 방식으로 설계함 ★★★
...
def build_feature_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ★★★ [확인 필요 5] product_id를 반드시 포함해서, 나중에 팀원1의 bayesian_rating과
...
def main():
    ...
    # ────────────────────────────────────────────────────────
    # ★★★ [확인 필요 6] 팀에서 합의한 출력 파일명으로 결정 ★★★
    # ────────────────────────────────────────────────────────
    output_path = '전처리_피처.csv'
